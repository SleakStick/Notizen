...
\beweis Sei $v_1,...,v_k$ eine Basis von $E_\lambda,v_k$ eine Basis von $E_\lambda$ und wir erweitern sie zu einer Basis $B=\lbrace v_1,...,v_k,v_{k+1},...,v_n\rbrace$ von $V$. Dann ist $[T]^B_B=\begin{pmatrix}\lambda I_k&C\\0&D\end{pmatrix}$ Dann ist
...
\textbf{Vermutung} \[V=\bigoplus_{\lambda=\text{Eigenwert}}\tilde{E_\lambda}\]
...
\textbf{Bemerkung 12.3.5} $(T-\lambda id_v)^{k-1}v=w$ ist ein Eigenvektor von $T$ mit Eigenwert $\lambda$
