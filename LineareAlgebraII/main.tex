...
Man merkt dass:
Copy
\satz{10.5.4} $T:V\rightarrow V$ mit Eigenwert $\lambda$. Dann gilt $g_\lambda \le a_\lambda$.
\beweis Sei $v_1, \ldots, v_k$ eine Basis von $E_\lambda$ und wir erweitern sie zu einer Basis $B = \{v_1, \ldots, v_k, v_{k+1}, \ldots, v_n\}$ von $V$. Dann ist
\[[T]^B_B = \begin{pmatrix} \lambda I_k & C \\ 0 & D \end{pmatrix}\]Dann ist
\[\det([T]^B_B - xI_n) = (\lambda - x)^k \cdot \det(D - xI_{n-k})\]
das bedeutet, dass $k \le a_\lambda$, da in $\det(D - xI_{n-k})$ auch eine Nullstelle vorkommen kann.
...
\subsection*{Jordansche Normalform} \definition{ Theorem} sei $\lambda\in K$ und $n\ge1$ der Jordanblock der länge $n$ und Eigenwert $\lambda$ ist folgende Matrix:\[J_n(\lambda)=\begin{pmatrix}\lambda&1&\smspc&0\\\smspc&\ddots&\ddots&\smspc\\0&\smspc&\lambda\end{pmatrix}\]
